module opdiv(
            ...
            case(next)
                ...
                begin
                        ...
                        if(signal_division_intern)begin
                            a_reg[30:0] <= a[31]  ?
                            {1'b0,~a[30:0]+1}: a[30:0];
                            b_reg[30:0] <= b[31]  ? {1'b0,~b[30:0]+1}: b[30:0];
                            a_reg[32:31]   <= (a[31] && a[30:0] == 0) ?
                            1 : 0;
    ...
    always_comb case(state) 
        IDLE                        :                               {next,next_in_ready_o,next_out_valid_o} = (in_valid_i && in_ready_o) ?
        {INITIALISE_AND_COUNTER_BITS,2'b00}:{IDLE,2'b10} ;
        ...
        SET_AK_MINUEND              :                               {next,next_in_ready_o,next_out_valid_o} = {LOOP,2'b00}                                                     
             ;
        LOOP                        :begin 
                                        if(compair)  {next,next_in_ready_o,next_out_valid_o} = !k ?
        {DONE,2'b01} : {UPDATE_MINUEND_RIGHT,2'b00}                            ;
        else                        {next,next_in_ready_o,next_out_valid_o} = !k ?
        {DONE,2'b01} : {UPDATE_MINUEND_LEFT ,2'b00 }                           ;
        end
    ...
    endcase
    // CORREÇÃO AQUI: Adicionado o operador ':' para garantir a sintaxe correta do 'assign'
    assign r_temp =  compair ?
    minuend - b_reg : minuend ;
                ...
                if(b_reg == '0)begin//mudou &&
                    ...
                    r = 0;
                        ...
                        case({a_signal,b_signal})
                            ...
                            2'b11:begin
                                    c = a_reg[31:0] == 32'h80000000 ?
                                Quatient[31:0] : {1'b0,Quatient[30:0]};
                                    r = ~r_temp+1;
                            end
                            2'b10:begin
                                    c = a_reg[31:0] == 32'h80000000 ?
                                ~Quatient[31:0] +1: {1'b1,~Quatient[30:0]+1};
                                    r = ~r_temp+1;
                            end
                            2'b01:begin 
